<!DOCTYPE html>
<html >
<head >
	<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.3.1/jquery.min.js"></script>
	<script type="text/javascript" src="logics.js"></script>
	<script type="text/javascript" charset="utf-8" src="js/featherlight.min.js" async></script>
	<script src="//code.jquery.com/jquery-1.11.1.min.js"></script>
	<link href="//maxcdn.bootstrapcdn.com/bootstrap/3.3.0/css/bootstrap.min.css" rel="stylesheet" id="bootstrap-css">
	<script src="//maxcdn.bootstrapcdn.com/bootstrap/3.3.0/js/bootstrap.min.js"></script>
	<link rel="stylesheet" type="text/css" href="style.css">
	<link rel="stylesheet" href="https://fonts.googleapis.com/css?family=Nothing+You+Could+Do|Poor+Story|Permenant+Marker|Fredericka+the+Great|Dekko|Londrina+Sketch|Hanalei|Sedgwick+Ave+Display|Rock+Salt|Cabin+Sketch|Nanum+Pen+Script|Sacramento|Cookies|Satisfy|Dancing+Script|Pacifico|Oregano|Jua">
	<link href="https://fonts.googleapis.com/css?family=Oleo+Script:400,700" rel="stylesheet">
    <link href="https://fonts.googleapis.com/css?family=Teko:400,700" rel="stylesheet">
    <link href="https://maxcdn.bootstrapcdn.com/font-awesome/4.7.0/css/font-awesome.min.css" rel="stylesheet">
	<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">


	
<div class="header" >
	<div class="navbar" id='nav' style="background-color:#111;background-image:linear-gradient(#444,#111);box-shadow:0 2px 1px #9c9c9c;">	
		<div id="bounce" class="hvr-bounce-to-right">
		<ul>

				<div class="dropdown">
					<li> <a href="AllPizzas.php"> <img src="Assets/pizz2.png"> Pizza</li>
						<div class="dropdown-content">
    				 		<a href="AllPizzas.php">All</a>
   					 		<a href="Chicken_Pizzas.php">Chicken</a>
   					 		<a href="Beef_Pizzas.php">Beef</a>
   					 		<a href="Speciality_Pizzas.php">Speciality</a>
						</div>

				</div>	

				<li>
				<a href="pasta.php"> 
                	<img src="Assets/pizz3.png">
					Pasta</li>


			
					<div class="dropdown">
				<li> <a href="AllAppetizer.php"><img src="Assets/pizz5.png">Appetizer</li>
					<div class="dropdown-content">
    				 		<a href="AllAppetizer.php">All</a>
   					 		<a href="Soup.php">Soup</a>
   					 		<a href="Salad.php">Salad</a>
   					 		<a href="Rolls.php">Rolls</a>
						</div>
			</div>

			
			<div class="logo" >				
					<li id="imglogo" >
						<a href="mainpage.php">
						<img  id="logoimg" src="Assets/logo4.png"  style="width: 100px; height: auto;">
						</a>
					</li>
				
			</div>
			

				<div class="dropdown">
					<li> <a href="AllDesserts.php"> <img src="Assets/pizz2.png"> Dessert</li>
						<div class="dropdown-content">
    				 		<a href="AllDesserts.php">All</a>
   					 		<a href="IceCream.php">Ice Cream</a>
   					 		<a href="Juices_Shakes.php">Juices And Shakes</a>
   					 		<a href="Cake.php">Cake</a>
						</div>

				</div>	


								
			<li>
			
			<a href="aboutus.php"> 

                	<img src="Assets/pizz6.png">
					About Us
					
				</li>
			<li>

	<a href="./cart.php">
					<div id="circle"><p id="number"></p></div>

                <img id="CartID"  src="Assets/shopping-cart.png"></a>
	
				</li>
					
		</ul>
		</div>
	</div>

</div>

